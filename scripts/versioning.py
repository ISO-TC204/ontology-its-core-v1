...
def version_iri(version: str) -> str:
    return f"{ONTOLOGY_NS}{normalize_semver(version)}"
...
def _replace_ttl_metadata(
    text: str, *, modified: str, version: str, prior: Optional[str]
) -> str:
    ...
    if re.search(r"^\s*owl:priorVersion\s+", text, flags=re.MULTILINE):
        if prior:
            text = sub_field("owl:priorVersion", f"<{version_iri(prior)}>", text)
        else:
            text = re.sub(
        ...
        text = re.sub(
            ...
            rf"\1 ;\n    owl:priorVersion               <{version_iri(prior)}> {term}",
            text,
            count=1,
            flags=re.MULTILINE,
        )
...
def stamp_ttl(
    ...
    for path in TTL_FILES:
        ...
        updated = _replace_ttl_metadata(
            original, modified=modified, version=vf.version, prior=prior
        )
        if updated != original:
